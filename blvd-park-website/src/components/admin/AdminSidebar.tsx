...
export default function AdminSidebar({ currentPath }: { currentPath: string }) {
  ...
  return (
    <>
      <div className="fixed inset-x-0 top-0 z-40 flex h-16 items-center justify-between border-b border-white/10 bg-[#0a0a0a] px-4 md:hidden">
        <a href="/admin" className="flex items-center gap-3 text-white">
          <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#1A5F36] font-bold">B</span>
          <span className="font-semibold">BLVD Park Admin</span>
        </a>
        <button
          type="button"
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-[min(18rem,86vw)] min-h-screen flex-col border-r border-white/5 bg-[#0a0a0a] transition-transform md:static md:z-auto md:w-64 md:translate-x-0 ${
